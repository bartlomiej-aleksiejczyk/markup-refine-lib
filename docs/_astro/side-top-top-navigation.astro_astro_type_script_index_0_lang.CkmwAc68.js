class p extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}connectedCallback(){this.render(),this.setupEventListeners(),this.revealComponent()}revealComponent(){this.style.display="block"}setupEventListeners(){const o=this.shadowRoot.querySelector("#sidebar-toggle"),e=this.shadowRoot.querySelector("#dismiss-button"),r=this.shadowRoot.querySelector(".primary-sidebar"),t=this.shadowRoot.querySelector(".overlay");o&&r&&t&&o.addEventListener("click",()=>{r.classList.toggle("drawer-open"),t.classList.add("visible"),document.body.style.overflowY="hidden"}),e&&r&&t&&e.addEventListener("click",()=>{r.classList.remove("drawer-open"),t.classList.remove("visible"),document.body.style.overflowY="visible"}),t&&r&&t.addEventListener("click",()=>{r.classList.remove("drawer-open"),t.classList.remove("visible"),document.body.style.overflowY="visible"})}render(){const o=document.documentElement,e=getComputedStyle(o),r=this.getAttribute("primary-topbar-color")||e.getPropertyValue("--color-secondary-darker").trim()||"hsl(0, 0%, 71%)",t=this.getAttribute("secondary-topbar-color")||e.getPropertyValue("--color-secondary-lighter").trim()||"hsl(0, 0%, 94%)",a=this.getAttribute("primary-sidebar-color")||e.getPropertyValue("--color-secondary").trim()||"hsl(0, 0%, 87%)",i=this.getAttribute("icon-color")||e.getPropertyValue("--color-text-secondary").trim()||"hsl(0, 0%, 33%)",s=this.getAttribute("icon-color-highlight")||e.getPropertyValue("--color-text-primary").trim()||"hsl(0, 0%, 20%)",n=this.querySelector('[slot="primary-topbar"]')!==null,l=this.querySelector('[slot="secondary-topbar"]')!==null,d=n?"":"hidden",c=l?"":"hidden";this.shadowRoot.innerHTML=`
      <style>
        :host {
