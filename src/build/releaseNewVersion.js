...
async function releaseNewVersion(versionType = "patch", customMessage = "") {
  try {
    ...
    const { latestTag, log: commitsSinceLast } = await getCommitsSinceLastTag();
    ...
    ${
      customMessage
        ? `- Notes: ${customMessage}`
        : commitsSinceLast
        ? `- Changes since ${latestTag || "initial commit"}:\n${commitsSinceLast
            .split("\n")
            .map((line) => `  ${line}`)
            .join("\n")}`
  ...
  try {
    ...
    const statusOutput = await execShellCommand("git status --porcelain");
    ...
    if (statusOutput.trim()) {
      ...
      console.log("Docs changes committed and pushed.");
    } else {
      console.log("No changes in docs/ to commit.");
    }
  } catch (error) {
...
async function getCommitsSinceLastTag() {
  try {
    const latestTag = (
      await execShellCommand("git describe --tags --abbrev=0")
    ).trim();
    const log = await execShellCommand(
      `git log ${latestTag}..HEAD --pretty=format:"- %s"`
    );
    return { latestTag, log: log.trim() };
  } catch (e) {
    console.warn(
      "⚠️ Could not fetch previous tag or commits. Maybe this is the first release?"
    );
    return { latestTag: null, log: "" };
  }
}
